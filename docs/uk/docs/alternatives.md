...
Насправді цілком звична річ використовувати Requests _всередині_ програми FastAPI.

Але все ж FastAPI черпав натхнення з Requests.

**Requests** — це бібліотека для _взаємодії_ з API (як клієнт), а **FastAPI** — це бібліотека для _створення_ API (як сервер).
...
Відповідна операція _роуту_ API FastAPI може виглядати так:
...
Це навіть не Python, NestJS — це фреймворк Node.JS JavaScript (TypeScript), натхненний Angular.
