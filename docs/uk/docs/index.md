...
<p align="center">
    <em>Фреймворк FastAPI, висока продуктивність, простий у вивченні, швидкий для написання коду, готовий до продакшину</em>
</p>
<p align="center">
...
FastAPI це сучасний, швидкий (високопродуктивний), вебфреймворк для створення API за допомогою Python 3.7+ на основі стандартних підказок типу Python.
...
* **Швидке написання коду**: Збільшить швидкість розробки функціоналу приблизно на 200%-300%. *
...
* **Надійний**: Ви матимете стабільний код готовий для продакшину з автоматичною інтерактивною документацією.
...
Якщо ви створюєте <abbr title="Command Line Interface">CLI</abbr> застосунок для використання в терміналі, замість веб-API зверніть увагу на <a href="https://typer.tiangolo.com/" class="external-link" target="_blank">**Typer**</a>.
...
* _Шлях_ `/items/{item_id}` має _параметер шляху_ `item_id` який має бути `int`.
...
Використовуючи стандартний **Python 3.7+**.
...
* Підтримку редактора, включаючи:
    ...
    * Перевірка типів.
* Перевірку даних:
    * Автоматичне видалення помилок, коли дані неправильні.
...
* Перевірить, чи є необов'язковий параметр запиту з назвою `q` (а саме `http://127.0.0.1:8000/items/foo?q=somequery`) для запитів `GET`.
    ...
    * За відсутності `None` він був би обов'язковим (як і тіло у випадку з `PUT`).
