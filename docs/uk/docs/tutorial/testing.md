...
Воно базується на [HTTPX2](https://pydantic.dev/docs/httpx2/), який, своєю чергою, спроєктований на основі Requests, тож він дуже знайомий та інтуїтивно зрозумілий.
...
Щоб використовувати `TestClient`, спочатку встановіть [`httpx2`](https://pydantic.dev/docs/httpx2/).
...
Використовуйте об'єкт `TestClient` так само як і `httpx2`.
...
Коли вам потрібно, щоб клієнт передав інформацію в запиті, але ви не знаєте, як це зробити, ви можете пошукати (Google), як це зробити в `httpx2`, або навіть як це зробити з `requests`, оскільки дизайн HTTPX2 базується на дизайі Requests.
...
Докладніше про передачу даних у бекенд (за допомогою `httpx2` або `TestClient`) можна знайти в [документації HTTPX2](https://pydantic.dev/docs/httpx2/).
