...
O marcador `@pytest.mark.anyio` informa o pytest que esta função de teste deve ser invocada de forma assíncrona:
...
!!! tip "Dica"
    Note que a função de teste é `async def` agora, no lugar de apenas `def` como antes quando estávamos utilizando o `TestClient`.
...
Como a função de teste é assíncrona agora, você pode agora chamar (e `esperar`) outras funções `async ` além de enviar requisições para a sua aplicação FastAPI em seus testes, exatamente como você as chamaria em qualquer outro lugar do seu código.
