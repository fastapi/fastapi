...
По по умолчанию Uvicorn будет использовать порт `8000` и не будет конфликтовать с сайтом документации, использующим порт `8008`.
...
Перейдите в таблицу кодов языков по ссылке указзанной выше, где найдёте, что кодом креольского языка является `ht`.
