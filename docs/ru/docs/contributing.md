...
Взоможно, для начала Вам стоит ознакомиться с основными способами [помочь FastAPI или получить помощь](help-fastapi.md){.internal-link target=_blank}.
...
Во избежание ошибок в дальнейших шагах, удостовертесь, что в Вашем виртуальном окружении установлена последняя версия `pip`:
...
После активации виртуального окружения, как было указано ранее, введите следуюшую команду:
...
И, так как при вводе вышеупомянутой команды был указан флаг `-e`, если Вы измените код локального FastAPI, то при следуещем запуске этого файла, он будет использовать свежую версию локального FastAPI, который Вы только что изменили.
...
Чтобы он сортировал их правильно, необходимо, чтобы FastAPI был установлен локально в Вашей среде, с помощью команды из разделе выше, использующей флаг `-e`.

## Документация

Прежде всего, убедитесь, что Вы настроили свою окружение, как описано выше, для установки всех зависимостей.
...
то по умолчанию Uvicorn будет использовать порт `8000` и не будет конфликтовать с сайтом документации, использующим порт `8008`.
...
* Добавтьте строку `docs/features.md` точно в то же место, как и в случае для английского, как-то так:
...
Перейдите в таблицу кодов языков по ссылке указзаной выше, где найдёте, что кодом креольского языка является `ht`.

Затем запустите скрипт, генерирущий директорию для переводов на новые языки:
...
Если возникла такая необходимость, отредактируйте `mkdocs.yml` для Вашего нового языка. Это будет выглядить как-то так:
...
После чего Вы сможет запустить сервер со всеми языками командой `serve`:
