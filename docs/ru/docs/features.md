...
Интерактивная API-документация и исследование пользовательских веб-интерфейсов. Поскольку этот фреймворк основан на OpenAPI, существует несколько вариантов документирования, 2 из которых включены по умолчанию.

* <a href="https://github.com/swagger-api/swagger-ui" class="external-link" target="_blank"><strong>Swagger UI</strong></a>, с интерактивным исследованием, вызывает и тестирует ваш API прямо из браузера.

![Swagger UI interaction](https://fastapi.tiangolo.com/img/index/index-03-swagger-02.png)

* Альтернативная API-документация в <a href="https://github.com/Rebilly/ReDoc" class="external-link" target="_blank"><strong>ReDoc</strong></a>.
...
Если вам нужно освежить знания, как использовать аннотации типов в Python (даже если вы не используете FastAPI), выделите 2 минуты и просмотрите короткий учебник: [Введение в аннотации типов Python¶
...
Весь фреймворк был продуман так, чтобы быть простым и интуитивно понятным в использовании, все решения были проверены на нескольких редакторах еще до начала разработки, чтобы обеспечить наилучшие условия при написании кода.
...
Как пример, ключ `price` внутри тела JSON (который может быть вложенным), которое приходит в запросе.

Больше не нужно писать неправильные имена ключей, метаться по документации или прокручивать окно вверх и вниз, чтобы узнать - использовали вы ранее `username` или `user_name`.
...
* Проверка значений для большинства (или всех?) **типов данных**  Python, включая:
    ...
    * Строковые (`str`) поля, с установленной минимальной и максимальной длиной.
...
Все схемы безопасности, определенные в OpenAPI, включая:

* HTTP Basic.
* **OAuth2** (также с **токенами JWT**). Ознакомьтесь с учебником [OAuth2 с JWT](tutorial/security/oauth2-jwt.md){.internal-link target=_blank}.
* Ключи API в:
    * Заголовках.
    * Параметрах запросов.
    * Куки и т.п.

Плюс все функции безопасности от Starlette (включая **session cookies**).
...
* Все зависимости могут запрашивать данные из запросов, содержания **аргументов операций пути** и из автоматической документации.
...
Или, другими словами, нет необходимости в них, импортируйте и используйте нужный вам код.

Любая интеграция разработана настолько простой в использовании (с зависимостями), что вы можете создать "плагин" для своего приложения в 2 строки кода, используя ту же структуру и синтаксис, что и для ваших *операций пути*.
...
**FastAPI** основан на <a href="https://www.starlette.io/" class="external-link" target="_blank"><strong>Starlette</strong></a> и полностью совместим с ней. Так что, любой дополнительный код Starlette, который у вас есть, будет также работать.
...
**FastAPI** основан на <a href="https://pydantic-docs.helpmanual.io" class="external-link" target="_blank"><strong>Pydantic</strong></a> и полностью совместим с ней. Так что, любой дополнительный код Pydantic, который у вас есть, будет также работать.
...
* Развлекайтесь с вашей **<abbr title="Интегрированное окружение для разработки, похожее на текстовый редактор">IDE</abbr>/<abbr title="программа проверяющая ошибки в коде">linter</abbr>/brain**:
    * Структуры данных pydantic - это всего лишь экземпляры классов, определённые вами. Автозаполнение, проверка кода, mypy и ваша интуиция - всё будет работать с вашими проверенными данными.
* **Быстродействие**:
    * В <a href="https://pydantic-docs.helpmanual.io/benchmarks/" class="external-link" target="_blank">тестовых замерах</a> Pydantic быстрее, чем другие проверенные библиотеки.
* Проверка **сложных структур**:
    ...
    * И валидаторы позволяют четко и легко определять, проверять и документировать сложные схемы данных в виде JSON Schema.
    * Вы можете иметь глубоко **вложенные объекты JSON** и все они будут проверены и прокомментированы.
* **Расширяемость**:
