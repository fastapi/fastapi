...
В таких случаях обычно возвращается **HTTP-код состояния** в диапазоне **400** (от 400 до 499).

Это аналогично кодам состояния 200 HTTP (от 200 до 299). Эти коды состояния "200" означают, что запрос завершился успешно.

Коды состояния в диапазоне 400 означают, что со стороны клиента произошла ошибка.

Помните все эти ошибки (и шутки) **"404 Not Found "**?
...
В данном примере, когда клиент запрашивает элемент по несуществующему ID, возникает исключение с кодом состояния `404`:
...
Если клиент запросит `http://example.com/items/foo` (`item_id` `"foo"`), то он получит код состояния HTTP 200 и ответ в формате JSON:
...
Но если клиент запросит `http://example.com/items/bar` (несуществующий `item_id` `"bar"`), то он получит код состояния HTTP 404 (ошибка "не найдено") и JSON-ответ в виде:
...
Здесь, если запросить `/unicorns/yolo`, то операция *операция пути* вызовет `UnicornException`.
...
А пока вы устраняете ошибку, ваши клиенты/пользователи не должны иметь доступа к внутренней информации о ней, так как это может привести к уязвимости в системе безопасности.
...
**FastAPI** имеет свои собственные `HTTPException`.

А класс ошибок **FastAPI** `HTTPException` наследует от класса ошибок Starlette `HTTPException`.

Единственное отличие заключается в том, что `HTTPException` в моде **FastAPI** позволяет добавлять заголовки, которые будут включены в ответ.
...
В данном примере, чтобы иметь возможность иметь оба `HTTPException` в одном коде, исключения Starlette переименованы в `StarletteHTTPException`:
