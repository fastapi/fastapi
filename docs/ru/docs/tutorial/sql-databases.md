...
Это очень простое и короткое руководство, поэтому, если вы хотите узнать о базах данных в целом или о SQL, разобраться с более продвинутым функционалом, то переходите по ссылке <a href="https://sqlmodel.tiangolo.com/" class="external-link" target="_blank">SQLModel docs</a>.
...
* `table=True` для SQLModel означает, что это *модель таблицы*, которая соответствует **таблице** в реляционной базе данных. Это не просто *модель данных* (в отличие от обычного класса в Pydantic).
...
Вместе запросом на создание героя мы получаем объект данных `HeroCreate`, и создаем на его основе объект модели таблицы `Hero`.
...
В коде мы получаем объект словаря `dict` с данными, переданными клиентом (т.е. **только данные, переданные клиентом**, исключая любые значения, которые могли бы быть там только потому, что они являются значениями по умолчанию). Для того чтобы сделать это, мы воспользуемся опцией `exclude_unset=True`. В этом главная хитрость. 🪄
...
В данном случае желание *`рефакторить всё`* остаётся неудовлетворенным. 😅
