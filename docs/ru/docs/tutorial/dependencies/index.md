...
Зависимости продолжат работу как и ожидалось, и **лучшая часть** в том что **информация о типе будет сохранена**, что означает что ваш редактор кода будет корректно обрабатывать **автокомплиты**, **встроенные ошибки**, и так далее. Тоже относится и к инструментам как `mypy`.
...
В интеррактивной документации будет вся информация по этим зависимостям тоже: 
...
Если вы посмотрите на фото, *функции обработки пути* объявлена всякий раз когда вычисляется путь и тогда **FastAPI** позаботится о вызове функции с корректными параметрами, извлекая информацию из запроса.
...
С системой системой Dependency Injection, вы можете также рассказать **FastAPI** что ваша *функции обработки пути* также "зависит" от чего то ещё, что должно быть извлечено перед  вашей *функции обработки пути*, и **FastAPI** позаботится об извлечении и инъекции результата.
...
Вы увидите примеры этого в следующих главах, о реляционных и NoSQL базах данных, безоастности и т.д.
...
Так же иерархическа система dependency injection очень простая для описывания и использывания, это все ещё очень сильно.

Вы можете описывать зависимости в очередь, они могут вызывать друг-друга.
...
К примеру, у вас есть 4 API эндпоинты (*операций пути*):
