...
Эта строка показывает URL-адрес, по которому приложение запущено на локальной машине.
...
"Путь" это часть URL, после первого символа `/`.
...
Директива `@app.get("/")` указывает **FastAPI**, что функция, следующая ниже, отвечает за обработку запросов, поступающих по адресу:
...
!!! info "`@decorator` Дополнительная информация"
    ...
    В нашем случае, этот декоратор сообщает **FastAPI**, что функция ниже соответствует **пути** `/` и использует **операцию** `get`.
...
* Пишем **декоратор операции пути** (`@app.get("/")`).
