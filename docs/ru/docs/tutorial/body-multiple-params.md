...
## Обьединение `Path`, `Query` параметров

Во-первых, конечно, вы можете объеденять параметры `Path`, `Query` и объявления тела запроса в своих функциях обработки, **FastAPI** автоматически определит, что с ними нужно делать.
...
## Различные параметры тела

В предыдущем примере, *path operations* ожидали JSON-тело с атрибутами параметра `Item`, например, такими как:
...
Но вы также можете объявить множество параметров тела, например `item` и `user`:
...
Таким образом, имена параметров будут использоваться в качестве ключей (имён полей) в теле запроса, и ожидать в следующем формате:
...
Точно так же, здесь используются `Query` `Path` для определения дополнительных данных, для query и path параметров, **FastAPI** предоставляет аналогичный инструмент - `Body`.

Например, расширяя предыдущую модель, вы можете решить, что хотите иметь еще один ключевой параметр `importance` в том же теле запроса, помимо параметров `item` и `user`.

Если вы объявите это, из-за единственного значения, **FastAPI** будет считать, что это query-параметр.

Но вы можете указать **FastAPI** обрабатывать его, как другой ключ тела запроса, используя `Body`:
...
Также, будет происходить преобразование типов данных, валидация, документирование и т.д.
...
Вы можете добавлять несколько body-параметров вашей *функции операции пути*, даже если запрос может содержать только одно тело.
