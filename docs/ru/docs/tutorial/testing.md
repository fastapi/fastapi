...
Тестирование основано на библиотеке [HTTPX2](https://pydantic.dev/docs/httpx2/), которая в свою очередь основана на библиотеке Requests, так что все действия знакомы и интуитивно понятны.
...
Для использования `TestClient` сначала установите [`httpx2`](https://pydantic.dev/docs/httpx2/).
...
Используйте объект `TestClient` так же, как Вы используете `httpx2`.
...
Если Вы не знаете, как передать информацию в запросе, можете воспользоваться поиском (Google) и задать вопрос: "Как передать информацию в запросе с помощью `httpx2`", можно даже спросить: "Как передать информацию в запросе с помощью `requests`", поскольку дизайн HTTPX2 основан на дизайне Requests.
...
Для получения дополнительной информации о передаче данных на бэкенд с помощью `httpx2` или `TestClient` ознакомьтесь с [документацией HTTPX2](https://pydantic.dev/docs/httpx2/).
