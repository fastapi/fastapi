...
С помощью `File` можно определить файлы, которые клиент будет загружать.
...
Создавайте параметры `File` так же, как вы это делаете для `Body` или `Form`:
...
Следует иметь в виду, что это означает, что все содержимое будет храниться в памяти. Это хорошо подходит для небольших файлов.

Однако есть несколько случаев, когда использование `UploadFile` может оказаться полезным.
...
## Необяхательная загрузка файлов
...
!!! note "Technical Details"
    ...
    **FastAPI** предоставляет те же `starlette.responses`, что и `fastapi.responses`, просто для удобства разработчика. Однако большинство доступных ответов поступает непосредственно из Starlette.
...
Используйте `File`, `bytes` и `UploadFile` для объявления файлов, которые будут загружаться и передаваться в виде данных формы.
