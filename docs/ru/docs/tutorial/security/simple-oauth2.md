...
/// tip | Подсказка
У экземпляра класса `OAuth2PasswordRequestForm` атрибут `scope` содержит список строк (а не длинную строку), полученных путём разделения длинной строки по пробелам. .
