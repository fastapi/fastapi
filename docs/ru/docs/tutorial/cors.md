...
В качестве списка источников можно указать подстановочный символ `"*"` ("wildcard"), чтобы обозначить все источники как разрешённые.
...
Вы также можете разрешить вашему бэкенду использовать:

* Учётные данные (заголовки Authorization, куки и т.п.).
* Отдельные HTTP-методы (`POST`, `PUT`) или все вместе, используя `"*"`.
* Отдельные HTTP-заголовки или все вместе, используя `"*"`.
...
### Предварительные (preflight) CORS-запросы
