...
FastAPI позволяет пользоваться **аннотациями типов** точно так же, как вы бы использовали их для **аргументов** функции. В аннотациях можно указывать схемы Pydantic, списки, словари, скалярные типы (такие, как int, bool и т.д.).
...
Допустим, вы хотите, чтобы ваша функция **возвращала словарь (dict)** или объект базы данных, но при этом **объявите выходной тип как схему Pydantic**. Тогда именно указанная схема будет использована для автоматической документации, валидации и т.п. для объекта, который вы вернули (например, словарь или объект базы данных).
...
Здесь мы объявили схему `UserIn`, которая хранит пользовательский пароль в открытом виде:
...
Далее мы используем нашу схему в аннотациях типа как для аргумента функции, так и для выходного значения:
