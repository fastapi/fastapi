...
Это приведёт к тому, что обьект `tags` преобразуется в список, не объявляя при этом тип элементов списка.

## Определение полей-списков с параметром типа

Однаков в Python есть специальный способ объявления списков с внутренними типами, или "параметрами типов":

### Импортируем `List` из модуля typing

В Python 3.9 и выше, вы можете использовать стандартный тип `list`, для объявления жтих аннотаций типов, как мы увидим ниже. 💡
...
### Объявление `list` с параметром типа

Для объявления типов, которые имеют параметры типа (внутренние типы), такие как `list`, `dict`, `tuple`:
...
Но затем мы подумали об этом, и поняли, что теги не должны повторяться, вероятно, они будут уникальными строками.
...
Таким образов, вы можете объявлять глубоко вложенные JSON "объекты" с определёнными именами атрибутов, типами и валидацией.
...
Также мы может использовать эту модель как тип атрибута:
...
Сделав такое объявление снова в **FastAPI**, вы получите:
...
Строка будет проверена на соответствие допустимому URL-адресу и документирована в JSON схему / OpenAPI.

## Атрибуты, содержащие списки подмоделей

Вы также можете использовать модели Pydantic в качестве подтипов `list`, `set` и т.д:
...
Данное решение предпологает (преобразование данных, валидация, документирование и т.д) наличие JSON тела в следующем формате:
...
Вы не могли бы получить такую поддержку редактора, если бы работали напрямую с `dict`, а не моделями Pydantic.
...
## Тела запросов произвольных `dict`-ов
...
Но вместе со всеми преимуществами:

* Поддержка редактора (автодополнение доступно везде!)
