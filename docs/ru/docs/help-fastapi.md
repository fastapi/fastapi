...
Или у Вас возникли трудности с **FastAPI** и Вам нужна помощь?
...
Можно связаться со <a href="https://tiangolo.com" class="external-link" target="_blank">мной (Себястьян Рамирез / `tiangolo`)</a>, автором фреймворка.

Вы можете:

* <a href="https://github.com/tiangolo" class="external-link" target="_blank">Подписаться на меня на **GitHub**</a>.
    * Посмотрите другие мои проекты с открытым кодом, которые могут помочь Вам.
    * Подписавшись на меня, Вы сможете узнать, что я создал новый проект с открытым кодом,.
* <a href="https://twitter.com/tiangolo" class="external-link" target="_blank">Подписаться на меня в **Twitter**</a> или в <a href="https://fosstodon.org/@tiangolo" class="external-link" target="_blank">Mastodon</a>.
    * Поделитесь со мной, как Вы используете FastAPI (я обожаю это).
    * Узнавайте, когда я делаю объявления и представляю новые инструменты.
...
* <a href="https://www.linkedin.com/in/tiangolo/" class="external-link" target="_blank">Связаться со мной в **Linkedin**</a>.
    * Узнавайте, когда я делаю объявления и представляю новые инструменты (правда, чаще всего я использую Twitter 🤷‍♂).
...
Вы можете посмотреть <a href="https://github.com/tiangolo/fastapi/issues" class="external-link" target="_blank">существующие проблемы</a> и попытаться помочь другим, чаще всего это вопросы, на которые, весьма вероятно, Вы уже знаете ответ. 🤓

Если Вы много помогаете людям с решением их проблем, Вы можете стать официальным [Экспертом FastAPI](fastapi-people.md#experts){.internal-link target=_blank}. 🎉

Только помните, самое важное при этом - быть добрым. Люди приходят со своими разочарованиями и часто задают вопросы не лучшим образом, но постарайтесь быть максимально доброжелательным. 🤗

Идея сообщества **FastAPI** в том, чтобы быть доброжелательным и гостеприимными. В то же время не допускайте издевательств или неуважительного поведения по отношению к другим. Мы должны заботиться друг о друге.

---

Вот как помочь другим с их проблемами:
...
* Ежели вопрос Вам непонятен, уточните больше **деталей**.

### Воспроизвести проблему

В большинстве случаев есть что-то связанное с **исходным кодом** вопрошающего.

И во многих случаях будет предоставлен только фрагмент этого кода, чего недостаточно для **воспроизведения проблемы**.
...
* Если на Вас нахлынуло великодушие, то можете попытаться **создать похожий пример** самостоятельно, основываясь только на описании проблемы. Но имейте в виду, что это может занять много времени и возможно сначала стоит позадавать вопросы для прояснения проблемы.

### Предложить решение

* После того, как Вы поняли вопрос, Вы можете дать **ответ**.

* Во многих случаях, следует понять **основную проблему и обстоятельства случая**, потому что может быть решение лучше, чем то, что пытались реализовать.
...
**Заметка**: Если Вы создаёте подобные запросы, то я попрошу Вас также оказать аналогичную помощь другим. 😉

## Проверить пул-реквесты

Вы можете помочь мне проверять пул-реквесты других участников.

И повторюсь, постаратесь быть доброжелательным. 🤗
...
* Также есть вероятность, что пул-реквест не актуален, так как проблему возможно решить **другим путём**. В таком случае Вы может указать на этот факт.

### Не переживать о стиле
...
* Проверьте и прочитайте код, посмотрите, имеет ли он смысл, **запустите его локально** и посмотрите, действительно ли он решает поставленную задачу.

* Затем используя **комментарий** расскажите, что Вы сделали проверку, тогда я буду знать, что Вы действительно проверили код.

!!! Информация
    К сожалению, я не могу так просто доверять пул-реквкстам, у которых уже есть несколько одобрений.
    ...
    Потому это действительно важно - проверять и запускать код, и при помощи комментария уведомлять меня, что Вы проделали эти действия. 🤓
...
* Многие пул-реквесты не имеют тестов, Вы можете **напомнить** о необходимости добавления тестов или даже **предложить** какие-либо свои тесты. Это одна из тех вещей, которые отнимают больше всего времени и Вы можете помочь с этим.

* Затем также добавьте комментарий, что Вы испробовали в ходе проверки, таким образом я буду знать, как Вы проверяли это. 🤓

## Создать пул-реквест

Вы можете [сделать вклад](contributing.md){.internal-link target=_blank} в код фреймворка использую пул-реквесты, например:
...
С другой стороны, в чатах тысячи пользователей, так что есть большие шансы в любое время найти там кого-то, с кем можно поговорить. 😄

## Спонсировать автора

Вы также можете оказать финансовую поддержку (мне) посредством <a href="https://github.com/sponsors/tiangolo" class="external-link" target="_blank">спонсорства через GitHub</a>.
...
Им также можно оказать спонсорскую поддержку:
