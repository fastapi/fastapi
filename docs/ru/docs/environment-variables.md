...
Вы можете **создавать** и использовать переменные окружения в **оболочке (терминале)**, не прибегая к помощи Python:
...
// Затем вы можете использовать его с другими программами, например
$ echo "Hello $MY_NAME"
...
Вы также можете создавать переменные окружения **вне** Python, в терминале (или любым другим способом), а затем **читать их в Python**.

Например, у вас может быть файл `main.py`:
...
Затем вы можете вызвать эту программу на Python:
...
Подробнее об использовании переменных окружения для работы с **настройками приложения** вы узнаете в [Advanced User Guide - Settings and Environment Variables](./advanced/settings.md){.internal-link target=_blank}.
...
Эта информация будет полезна при изучении [Virtual Environments](virtual-environments.md){.internal-link target=_blank}.

## Вывод

Благодаря этому вы должны иметь базовое представление о том, что такое **переменные окружения** и как их использовать в Python.

Подробнее о них вы также можете прочитать в <a href="https://en.wikipedia.org/wiki/Environment_variable" class="external-link" target="_blank">Wikipedia for Environment Variable</a>.

Во многих случаях не очень очевидно, как переменные окружения могут быть полезны и применимы сразу же. Но они постоянно появляются в различных сценариях разработки, поэтому знать о них полезно.

Например, эта информация понадобится вам в следующем разделе, посвященном [Virtual Environments](virtual-environments.md).
