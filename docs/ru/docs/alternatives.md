...
Что вдохновило на создание **FastAPI**, сравнение его с другими альтернативами и чему он научился у них.
...
Он использовался для создания таких продуктов, как, например, Instagram.

Django довольно тесно связан с реляционными базами данных (такими как MySQL или PostgreSQL), потому использовать NoSQL базы данных (например, Couchbase, MongoDB, Cassandra и т.д.) в качестве основного хранилища непросто.
...
Он очень прост, его изучение интуитивно понятно, хотя в некоторых местах документация несколько техническая.

Flask часто используется и для приложений, которым не нужна база данных, управление пользователями или любая из множества функций, предварительно встроенных в Django.
...
!!! check "Идеи для **FastAPI**"
    ...
    Должна быть простая и лёгкая в использовании система перенаправления запросов.
...
!!! check "Идеи для **FastAPI**"
    ...
    * Должны быть разумные настройки по умолчанию и широкие возможности настройки.
...
Другая немаловажная функция API - <abbr title="чтение и преобразование данных в объекты Python">разбор и анализ</abbr> данных из входящих запросов.
...
Так это и работает для Flask, Starlette, Responder и т.п.
...
Это даже не Python. NestJS - этот фреймворк написанный на JavaScript (TypeScript), основанный на NodeJS и вдохновлённый Angular.

Он позволяет получить что-то похожее на то, что можно сделать с помощью Flask-apispec.
...
Sanic был один из первых чрезвычайно быстрых Python-фреймворков основанных на `asyncio`.
...
!!! check "Идеи для **FastAPI**"
    ...
    Для это **FastAPI** основан на Starlette, самом быстром из доступных фреймворков (по замерам непричастных лиц).
...
Или эти функции должны быть встроены во фреймворк, сконструированный поверх Falcon, как Hug.
Такая же особенность и в других фреймворках, вдохновлённых идеей Falcon, использовать только один объект запроса и один объект ответа.

!!! check "Идея для **FastAPI**"
    Объявлять параметры ответа в функциях, как в Hug.
...
Система внедрения зависимостей требует предварительной регистрации и зависимости определяются, как объявления типов.
Из-за чего невозможно объявить более одного "компонента" (зависимости), который предоставляет определенный тип.
...
В нём была автоматическая проверка и сериализация данных и генерация схемы OpenAPI основанные на подсказках типов в нескольких местах.
...
Я не смог использовать его в полноценном проекте, так как в нём были проблемы при интеграции <abbr title="Авторизация и соответствующие допуски к операциям пути (эндпоинтам)">функций безопасности</abbr>, со схемой OpenAPI , из-за которых я не мог вставить все функции, которые у меня были в полноценных генераторах проектов на основе Flask-apispec.
...
!!! check "**FastAPI** использует Starlette"
    В качестве ядра веб-сервиса для обработки запросов, добавив некоторые функций сверху.
