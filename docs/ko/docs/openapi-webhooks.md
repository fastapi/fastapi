...
일반적으로 **코드에서** 웹훅으로 보낼 메시지, 즉 요청의 **바디(body)**를 정의합니다.

그리고 앱이 해당 요청이나 이벤트를 보낼 **시점**을 정의합니다.

그리고 **사용자**는 앱으로 요청을 보낼 **URL**을 정의합니다(예: 웹 대시보드에서 설정).

웹훅의 URL을 등록하는 방법과 이러한 요청을 실제로 전송하는 코드에 대한 모든 로직은 유저에게 달려 있습니다. 원하는대로 **자신의 코드**를 작성하면 됩니다.
