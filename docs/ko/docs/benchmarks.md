...
* **FastAPI**:
    ...
    * FastAPI는 Starlette에 추가적으로 더 많은 기능을 제공합니다. API를 구축할 때 거의 항상 필요한 데이터 검증 및 직렬화와 같은 기능들이 포함되어 있습니다. 그리고 이를 사용하면 자동 문서화 기능도 제공됩니다(자동 문서화는 응용 프로그램 실행 시 오버헤드를 추가하지 않고 시작 시 생성됩니다).
