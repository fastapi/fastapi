...
**"정규표현식"** 아이디어에 대해 상실감을 느꼈다면 걱정하지 않아도 됩니다. 많은 사람에게 어려운 주제입니다. 아직은 정규표현식 없이도 많은 작업들을 할 수 있습니다.
...
!!! tip "팁"
    위의 예와 같이 `list` 형으로 쿼리 매개변수를 선언하려면 `Query`를 명시적으로 사용해야 합니다. 그렇지 않으면 요청 본문으로 해석됩니다.

대화형 API 문서는 여러 값을 허용하도록 업데이트 됩니다:
...
!!! note "참고"
    ...
    일부는 아직 선언 된 추가 정보를 모두 표시하지 않을 수 있지만, 대부분의 경우 누락된 기능은 이미 개발 계획이 있습니다.
...
매개변수가 `item-query`이길 원한다고 상상해 봅시다.
...
숫자와 같은 다른 타입들에 대한 검증을 어떻게 선언하는지 보려면 다음 장을 확인하세요.
