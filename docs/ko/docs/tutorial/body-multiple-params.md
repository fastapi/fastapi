...
이전 예제에서 보듯이, *path* 연산은 아래와 같이 `Item` 속성을 가진 JSON 본문을 예상합니다:
...
하지만, 또한 다중 본문 매개변수를 선언할 수 있습니다. 예를 들어 `item`과 `user`를 갖고 있다고 생각해보겠습니다.:
...
복잡한 데이터의 유효성 검사를 수행할 수 있고, OpenAPI 형태로 자동 문서화를 수행할 것입니다.
...
이것은 다시 데이터 타입으로 변환되거나 검증되고, 문서화 될 수 있습니다.
...
!!! info "정보"
    `Body` 또한 동일한 유효성 검증과 `Query`, `Path` 그리고 이후에 볼 다른 것들처럼 메타데이터 매개변수를 갖고 있습니다. 
...
하지만, 만약 모델 내용에 `item `키를 가진 JSON으로 예측하길 원한다면, 추가적인 `Body` 매개변수를 선언한 것처럼 `Body`의 특별한 매개변수인 `embed`를 이용해 사용할 수 있습니다:
