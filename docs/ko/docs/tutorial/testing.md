...
테스트를 위한 함수는 `async def` 가 아니라 `def` 로 작성됨을 알려드립니다.
...
요청 설계가 `httpx` 의 설계를 기반으로 하고 있으므로, 클라이언트(TestClient)에 정보를 어떻게 전달해야 할지 모를 경우, 검색(Google)을 통해 `httpx` 에서 어떻게 처리하는지 찾아보면 알 수 있습니다.
