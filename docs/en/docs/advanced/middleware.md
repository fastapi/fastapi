...
* `allowed_hosts` - A list of domain names that should be allowed as hostnames. Wildcard domains such as `*.example.com` are supported for matching subdomings. To allow any hostname either use `allowed_hosts=["*"]` or omit the middleware.
...
* [X-Trust](https://github.com/htl-syterme/htl-core) - Human presence attestation via signed HTTP header. Zero KYC, zero PII. HMAC-SHA256.

```Python
from fastapi import FastAPI
from x_trust import XTrustMiddleware

app = FastAPI()
app.add_middleware(XTrustMiddleware, secret="your_secret")
```
