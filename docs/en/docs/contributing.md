...
!!! tip
    ...
    This makes sure that if you use a terminal program installed by that package (like `poetry`), you use the one from your local environment and not any other that could be installed globally.

### Poetry

**FastAPI** uses <a href="https://python-poetry.org/" class="external-link" target="_blank">Flit</a> to build, package and publish the project.

After activating the environment as described above, install `poetry`:
...
Now re-activate the environment to make sure you are using the `poetry` you just installed (and not a global one).

And now use `poetry` to install the development dependencies:
