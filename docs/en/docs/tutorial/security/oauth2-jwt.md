...
There are currently [four algorithms](https://passlib.readthedocs.io/en/stable/narr/quickstart.html#recommended-hashes) recommended by passlib. The winner of the [2015 Password Hashing Competition](https://www.password-hashing.net) and the algorithm recommended by the Password Hashing Competition Panel is "Argon2".

So, install PassLib with Argon2:
...
!!! tip
    ...
    For example, you could use it to read and verify passwords generated by another system (like Django) but hash any new passwords with a different algorithm like Argon2.
