...
If the client requests `http://example.com/items/foo` (an `item_id` `"foo"`), they will receive an HTTP status code of 200, and a JSON response of:
...
But if the client requests `http://example.com/items/bar` (a non-existent `item_id` `"bar"`), they will receive an HTTP status code of 404 (the "not found" error), and a JSON response of:
