...
Using different path converters, such as `{item_id:int}` and `{item_id:str}`, can
also produce the same OpenAPI path. FastAPI will warn you about this collision,
and you should use distinct path parameter names to avoid documenting a different
handler than the one selected at runtime.
