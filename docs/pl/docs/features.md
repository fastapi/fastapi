...
### Oparcie o otwarte standardy

* <a href="https://github.com/OAI/OpenAPI-Specification" class="external-link" target="_blank"><strong>OpenAPI</strong></a> dla tworzenia API, w tym deklaracji <abbr title="znane również jako: paths, endpoints, routes">ścieżek</abbr>, <abbr title="znane również jako metody HTTP, takie jak POST, GET, PUT, DELETE">operacji</abbr>, parametrów, <abbr title="po angielsku: body requests">ciał zapytań</abbr>, bezpieczeństwa, itp.
...
Interaktywna dokumentacja i webowe interfejsy do eksploracji API. Z racji tego, że framework bazuje na OpenAPI, istnieje wiele opcji, z czego 2 są dołączone domyślnie.
...
Wszystko opiera się na standardowych deklaracjach typu **Python 3.6** (dzięki Pydantic). Brak nowej składni do uczenia. Po prostu zwykły, współczesny Python.

Jeśli potrzebujesz szybkiego przypomnienia jak używać deklaracji typów w Pythonie (nawet jeśli nie korzystasz z FastAPI), sprawdź krótki samouczek: [Python Types](python-types.md){.internal-link target=_blank}.

Piszesz zwykły kod Pythona z typami:
...
A potem można ich użyć w taki sposób:
...
Ostatnia ankieta wśród programistwów Pythona jasno wskazuje, że <a href="https://www.jetbrains.com/research/python-developers-survey-2017/#tools-and-features" class="external-link" target="_blank">najczęściej używaną funkcjonalnością jest autouzupełnianie w edytorze</a>.
...
Otrzymasz uzupełnienie nawet w miejscach, w których wydawało ci się to niemożliwe. Na przykład klucz `price` w treści JSON (który mógł być zagnieżdżony), pochodzącej z zapytania.
...
Wszystko posiada sensowne **domyślne wartości**, z dostępnymi opcjonalnymi konfiguracjami. Wszystkie parametry możesz dostroić, aby robiły to, czego potrzebujesz oraz do zdefiniowania API, które tworzysz.

Ale wyjściowo wszystko **"po prostu działa"**.
...
* <abbr title="po angielsku: HTTP basic auth">Podstawowe uwierzytelnianie HTTP</abbr>.
...
* Wszystkie zależności mogą wymagać danych w żądaniach, rozszerzenia ograniczeń **<abbr title="po angielsku: path operations">operacji na ścieżce</abbr>** i automatycznej dokumentacji.
* **Automatyczna walidacja** parametrów *operacji na ścieżce* zdefiniowanych w zależnościach.
* Wsparcie dla złożonych systemów uwierzytelniania użytkowników, **połączeń z bazami danych**, itp.
* Bazy danych, frontend, itp. **bez kompromisów**, ale wciąż łatwe do integracji.
...
* 100% <abbr title="Deklaracje typów Python - dzięki nim twój edytor i zewnętrzne narzędzia mogą zapewnić Ci lepsze wsparcie">adnotacja typów</abbr> kodu źródłowego.
* Używany w aplikacjach produkcyjnych.

## Cechy Starlette

**FastAPI** jest w pełni kompatybilny z (oraz bazuje na) <a href="https://www.starlette.io/" class="external-link" target="_blank"><strong>Starlette</strong></a>. Tak więc każdy dodatkowy kod Starlette, który posiadasz, również będzie działać.
...
* Wydarzenia podczas <abbr title='po angielsku: startup'>uruchomienia</abbr> i <abbr title='po angielsku: shutdown'>zamknięcia</abbr>.
* Klient testowy zbudowany na HTTPX.
...
Wliczając w to zewnętrzne biblioteki, również oparte o Pydantic, takie jak <abbr title="Mapowanie obiektowo-relacyjne. Po angielsku: Object-Relational Mapper">ORM</abbr>, <abbr title="Mapowanie obiektowo-dokumentowe. Po angielsku: Object-Document Mapper">ODM</abbr> dla baz danych.
...
Dzięki **FastAPI** otrzymujesz wszystkie funkcje **Pydantic** (ponieważ FastAPI bazuje na Pydantic do całej obsługi danych):

* **Bez rozwalonego mózgu**:
    * Brak nowego mikrojęzyka do definiowania schematu, którego trzeba się nauczyć.
    * Jeśli znasz typy Pythona, to wiesz jak używać Pydantic.
* Dobrze współpracuje z Twoim **<abbr title='Skrót od "Integrated Development Environment", zintegrowane środowisko developerskie'>IDE</abbr>/<abbr title="Program, który sprawdza Twój kod pod kątem błędów">linterem</abbr>/mózgiem**:
...
* Walidacja **złożonych struktur**:
    * Wykorzystanie hierarchicznych modeli Pydantic, `List`, `Dict` z Pythonowego modułu `typing`,  itp.
