...
* 使用 <a href="https://json-schema.org/" class="external-link" target="_blank"><strong>JSON Schema</strong></a> （因為 OpenAPI 本身就是基於 JSON Schema）自動生成數據模型文件。
...
* 所有 **自動化處理** 都由框架完成。
* 依賴項不僅能從請求中提取數據，還能 **對 API 的路徑操作進行強化**，並自動生成文檔。
...
**FastAPI** 完全兼容且基於 <a href="https://www.starlette.io/" class="external-link" target="_blank"><strong>Starlette</strong></a>。所以，你有其他的 Starlette 程式碼也能正常工作。FastAPI 繼承了 Starlette 的所有功能，如果你已經知道或者使用過 Starlette，大部分的功能會以相同的方式工作。
...
通過 **FastAPI** 你可以獲得所有 **Pydantic** 的特性 (FastAPI 基於 Pydantic 做了所有的數據處理)：
...
* **可擴展**：
    * Pydantic 允許定義自訂數據類型，或者你可以使用帶有 validator 裝飾器的方法來擴展模型中的驗證功能。
* 100% 測試覆蓋率。
