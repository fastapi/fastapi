...
!!! info
    `GET` オペレーション (HTTP メソッド) を使用してリクエストボディを送ることはできません。

    データを送るには、`POST` (もっともよく使われる)、`PUT`、`DELETE` または `PATCH` を使わなければいけません。
...
エディターによる型ヒントと補完が関数内で利用できます (Pydanticモデルではなく `dicct` を受け取ると、同じサポートは受けられないかもしれません):
