...
**型ヒント**は変数の<abbr title="例えば：str、int、float、bool">型</abbr>を宣言できる特殊の文法です。

変数を宣言することによって、エディターやツールからよりいいサポートを受けることができます。

この文章はPython型ヒントについての**クイックチュートリアル / おさらい**です。**FastAPI**を使うために型ヒントについて必要最小限の知識しか含まれていません。

**FastAPI**はすべて型ヒントに基づいて構築されています。このことによってたくさんのアドバンテージと利便性をもたらした。

**FastAPI**を使わなくても，型ヒントを知ることはどこかで役に立てるでしょう。

!!! note "備考"
    もしあなたはすでにPythonのエキスパートで、型ヒントについてよく知っているなら、次の章に移しましょう。
...
* メソット`title()`でそれぞれの引数の最初のアルファベットを大文字に変換します。
...
でも、ゼロからこのプログラムを書くところを想像してください。

ある時に、関数を定義し、引数を用意した...。

次に、”最初のアルファベットを大文字に変換するメソット”を呼び出そうとします。

あれ、待って、そのメソットは何でしたっけ、`upper`ですか？それとも `uppercase`？`first_uppercase`？`capitalize`ですか？
...
通常なら型ヒントを入れるかと入れないかで実行結果を変えることはありません。

では、もう一度この関数を作っているところを想像してください。今度は型ヒントを入れました。

同じところで`Ctrl+Space`を押して自動補完を呼び出そうとすると、こんなことが見えます：
...
このすでに型ヒントを入れた関数を見てください：

```Python hl_lines="1"
{!../../../docs_src/python_types/tutorial003.py!}
```

エディターはすでに変数の型を知っているため、コードの補完だけではなく、エラーをチェックすることもできます。

<img src="https://fastapi.tiangolo.com/img/python-types/image04.png">

これを修復しなければならないから、`str(age)`を使って`age`を文字列に変換します。
...
ここまで紹介したのが、型ヒントを宣言する主な場面です。関数の引数として。

これも**FastAPI**において、型ヒントを使う主な場面です。

### シンプルタイプ

`str`型だけではなく、すべてのPythonの標準な型を宣言することがでます。
...
このモジュールはこういった型をサポートするためだけにあります。
...
上のプログラムの意味は”変数`items`は`list`型であり、かつその中身のすべての要素は`str`型です”

こうすると、リストの中の要素を処理しているときも、エディターがサポートしてくれます。

<img src="https://fastapi.tiangolo.com/img/python-types/image05.png">

タイプを入れないとこんことは実現できない。

ここで注意すべきのは変数`item`はリスト`items`の一つの要素です。
...
`Optional`を使ってとある変数には型があると宣言することができます。しかし、それは”オプショナル”なことです。つまり、その変数の値は`None`であることも許されます。
...
`Person`というクラスがあると仮定しましょう、nameというフィールドを持ちます。
...
<a href="https://pydantic-docs.helpmanual.io/" class="external-link" target="_blank">Pydantic</a>はデータ妥当性検証をするためのPyhtonライブラリです。

データの”構造”をフィールドを持つクラスとして宣言することができます。

それぞれのフィールドには型があります。

次に、いくつかの値をもってそのクラスのインスタンスを作ります。このモジュールは値の妥当性を検証し、適切な型に変換して（必要であれば）、すべてのデータを含むオブジェクトを返します。

そうすると、このオブジェクトにおいてすべてのエディターサポートを受けることができます。

次に例はPydanticの公式ドキュメントからのものです：
...
!!! info "情報"
    Pydanticについて詳しく知りたいなら<a href="https://pydantic-docs.helpmanual.io/" class="external-link" target="_blank">ドキュメントをチェックしてください</a>。
...
...さらに**FastAPI**はその宣言を用いて以下のことを行なっています：
...
何より、Python標準型を用いてひとつのどころにだけ定義すれば（たくさんのクラスとデコレーターを入れると代わりに）、**FastAPI**はたくさんの仕事をやってくれます。

!!! info "情報"
    すでにチュートリアルをすべて終えて、型についてもっと知りたいと思って戻ってきた人には、<a href="https://mypy.readthedocs.io/en/latest/cheat_sheet_py3.html" class="external-link" target="_blank">`mypy`の”cheat sheet”</a>は良い資料になるでしょう。