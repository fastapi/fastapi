...
### 自動ドキュメント生成
対話的なAPIドキュメントと探索的なwebユーザーインターフェースを提供します。OpenAPIを基にしたフレームワークのようないくつかのオプションがあります。デフォルトで2つ含まれています。
...
（FastAPIを使用しない場合でも）Pythonの型の使用方法について簡単な復習が必要な場合は、ショートチュートリアル（[Python Types](python-types.md){.internal-link target=_blank}）を参照してください。
...
すべてに適切な**デフォルト**があり、どこにでもオプションの構成があります。必要なことを実行し、必要なAPIを定義するためにすべてのパラメーターを調整できます。
