...
コンピュータ/プログラム🤖は再び待機する機会があるときや、その時点でそれ🤖が行っていたすべての作業が完了するたびに戻ってきます。そして、それ🤖が待っていた処理が終わっているか否か、確認します。

次に、それ🤖が最初のタスク (「遅いファイル📝」としましょう) を受け付けて、それを使う必要があったことを続けます。

この「何かを待つ」とは、通常、以下の様なものを待つ、相対的に「遅い」(プロセッサとRAMメモリの速度に比べて)<abbr title="インプットとアウトプット">I/O</abbr> 操作を指します:
...
なので、あなたはあなたの好きな人😍が話し終えるのを待って (現在の仕事⏯ / 処理中のタスクを終了します🤓)、優しく微笑んで、ハンバーガーを貰いに行こうと言います⏸。
...
**コルーチン**は、`async def` 関数によって返されるものを指す非常に凝った用語です。これは、開始できて、いつか終了する関数のようなものであるが、内部に `await` があるときは内部的に一時停止⏸されることもあるものだとPythonは認識しています。
