...
データベース、API、ファイルシステムなどと通信し、`await` の使用をサポートしていない (現在のほとんどのデータベースライブラリに当てはまります) サードパーティを使用している場合、次の様に、単に `def` を使用して通常通り *path operation 関数* を宣言してください:
...
アプリケーションが (どういうわけか) 他の何とも通信せず、応答するのを待つ必要がない場合は、`async def` を使用して下さい。
...
**備考**: *path operation 関数*に必要なだけ `def` と `async def` を混在させ、それぞれに最適なオプションを使用して定義できます。FastAPIは正しい処理を行います。
...
現代版のPythonは「**非同期コード**」を、「**コルーチン**」と呼ばれるものと、**`async` と `await`** 構文でサポートしています。 
...
非同期コードとは、言語💬がコンピュータ/プログラム🤖にコード内のどこかの箇所でそれ🤖がどこかの *何か* が終了するのを待たなければならないと伝える手段を持っていることを意味します。*何か* は「遅いファイル📝」と呼ばれているとしましょう.

したがって、コンピュータは「遅いファイル📝」が終了するまで、他の処理ができます。

コンピュータ/プログラム🤖は再び待機する機会があるときや、その時点でそれ🤖が行っていたすべての作業が完了するたびに戻ってきます。そして、それ🤖が待っていた処理が終わっているのか、否か、確認します。

次に、それ🤖が最初のタスク (「遅いファイル📝」としましょう) を受け付けて、それを使用しなければならなかったことを続けます。

この「何かを待つ」とは、通常、以下の様なものを待つような、相対的に「遅い」(プロセッサとRAMメモリの速度に比べて)<abbr title="インプットとアウトプット">I/O</abbr> 操作を指します:
...
その代わりに、「非同期」システムであることにより、いったん終了すると、タスクは、コンピュータ/プログラムが既に開始した処理がすべて完了するのを少し (数マイクロ秒) 待って、結果を受け取りに戻ってきます。そして、処理を継続します。
...
レジ係💁はキッチンの男👨‍🍳に何か言ったので、その男はあなたのハンバーガー🍔を準備しなければならないと知っています (彼は現在、前のお客さんの商品を準備していますが)。

レジ係💁はあなたに番号札を渡します。

待っている間、好きな人😍と一緒にテーブルを選びに行き、好きな人😍と長い間座って話します (注文したハンバーガーは非常に豪華で、準備に少し時間がかかるので✨🍔✨)。

好きな人😍とテーブルに座っているとき、ハンバーガー🍔を待つ間、あなたの好きな人がどれほど素晴らしく、かわいくてスマートであるか✨😍✨を賞賛しながら時間を費やすことができます。
...
あなたがこのストーリーのコンピュータ/プログラム🤖であると想像してみてください。

列にいる間、あなたはアイドル状態です😴、あなたの番を待って、何も「生産的」なことをせず、ただ自分の番を待っています。しかし、レジ係💁は注文を受け取るだけなので (準備はしていない)、列は高速です。したがって、何も問題ありません。

それから、あなたの番になったら、実に「生産的な」作業を行います🤓、メニューを処理し、必要なものを決定し、好きな人😍の好みを選び、料金を支払い💸、正しいキャッシュまたはカードを渡したか確認し、正しく清算されたことを確認し、注文が正しく通っているかなどを確認します。
...
次に、レジ係💁は、「ハンバーガーの準備ができました🍔」と言って、カウンターのディスプレイに番号を表示しますが、表示番号があなたの番号に変わっても、すぐに狂ったように飛んで行くようなことはありません。それぞれ自分の番号札を持っているので、ハンバーガー🍔を盗む人がいないことは皆知っています。

なので、あなたはあなたの好きな人😍が話し終えるのを待って (現在の仕事⏯ / 処理中のタスクを終了します🤓)、優しく微笑んで、ハンバーガーを貰いに行こうと言いいます⏸。

次に、カウンターへ、いま完了した最初のタスク⏯へ向かい、ハンバーガー🍔を受け取り、感謝の意を表して、テーブルに持っていきます。これで、カウンターとのやり取りのステップ/タスクが完了しました⏹。これにより、「ハンバーガーを食べる🔀⏯」という新しいタスクが作成されます。しかし、前の「ハンバーガーを取得する」というタスクは終了しました⏹。
...
8人のレジ係がそれぞれ自分で注文を受け、次の注文を受ける前にハンバーガーを準備するので、あなたの前の人達はカウンターを離れずに、ハンバーガー🍔ができるのを待っています🕙。
...
あなたとあなたの好きな人は忙しいので、誰かがあなたの前に来て、彼らが到着したとき🕙にハンバーガーを取ることを許せず、あなたはあなたの好きな人😞に注意を払えません。

これは「同期」作業であり、レジ係/料理人👨‍🍳と「同期」します。レジ係/料理人👨‍🍳がハンバーガー🍔を完成させてあなたに渡すまで待つ🕙必要があり、ちょうどその完成の瞬間にそこにいる必要があります。そうでなければ、他の誰かに取られるかもしれません。

その後、カウンターの前で長い時間待ってから🕙、レジ係/料理人👨‍🍳がハンバーガー🍔を渡しに来てくれます。
...
この並列ハンバーガーのシナリオでは、あなたは2つのプロセッサーを備えたコンピュータ/プログラム🤖 (あなたとあなたの好きな人😍) であり、両方とも待機🕙していて、彼らは「カウンターで待機🕙」することに専念しています⏯。

ファストフード店には8つのプロセッサー (レジ係/料理人) 👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳があります。 一方、並行ハンバーガー店には2人 (レジ係と料理人) 💁👨‍🍳しかいなかったかもしれません。
...
並行処理は並列処理とは異なります。多くの待機を伴う**特定の**シナリオに適しています。そのため、一般に、Webアプリケーション開発では並列処理よりもはるかに優れています。しかし、すべてではありません。

なので、バランスをとるために、次の物語を想像して下さい:

> 大きくて汚れた家を掃除する必要があります。
...
あなたはハンバーガーの例のように、最初はリビングルーム、次にキッチンのようにターンを持つことができますが、あなたは何も待機🕙していないので、掃除とクリーニングだけをしていれば、ターンは何にも影響しません。

ターンの有無に関係なく (並行に) 同じ時間がかかり、同じ量の作業が行われることになるでしょう。

しかし、この場合、8人の元レジ係/料理人/現役清掃員👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳👨‍🍳を手配できて、それぞれ (さらにあなたも) が家の別々の場所を掃除できれば、追加の助けを借りて、すべての作業を**並列**に行い、はるかに早く終了できるでしょう。

このシナリオでは、清掃員 (あなたを含む) のそれぞれがプロセッサーとなり、それぞれの役割を果たします。
...
* **機械学習**: 通常、多くの「行列」と「ベクトル」の乗算が必要です。 数値を含む巨大なスプレッドシートを考え、それらすべてを同時に乗算。
* **ディープラーニング**: これは機械学習のサブフィールドであるため、同じことが当てはまります。 乗算するのは数値の単一のスプレッドシートではなく、それらの膨大なセットで、多くの場合、それらのモデルを構築および/または使用するために特別なプロセッサを使用します。
...
本番環境でこの並列処理を実現する方法については、[デプロイ](deployment.md){.internal-link target=_blank}に関するセクションを参照してください。
...
同じ (またはほとんど同じ) 構文が、最近のバージョンのJavaScript (ブラウザおよびNodeJS) にも最近組み込まれました。

しかし、その前は、非同期コードの処理はかなり複雑で難解でした。

以前のバージョンのPythonでは、スレッドや<a href="http://www.gevent.org/" class="external-link" target="_blank">Gevent</a>が利用できました。しかし、コードは理解、デバック、そして、考察がはるかに複雑です。
...
**コルーチン**は、`async def` 関数によって返されるものを指す非常に凝った用語です。 これは、開始できて、いつか終了する関数のようなものであるが、内部に `await` があるときは内部的に一時停止⏸されることもあるというものだとPythonは認識しています。
...
(Starletteを介して) FastAPIに力を与えて、印象的なパフォーマンスを実現しているものはこれだけです。
...
上記の方法で機能しない別の非同期フレームワークから来ており、小さなパフォーマンス向上 (約100ナノ秒) のために通常の `def` を使用してささいな演算のみ行う *path operation 関数* を定義している場合は、**FastAPI**ではまったく逆の効果になることに注意してください。このような場合、*path operation 関数* がブロッキング<abbr title="入力/出力: ディスクの読み取りまたは書き込み、ネットワーク通信。">I/O</abbr>を実行しないのであれば、`async def` の使用をお勧めします。

それでも、どちらの状況でも、**FastAPI**が過去のフレームワークよりも (またはそれに匹敵するほど) [依然として高速になる](/#performance){.internal-link target=_blank}可能性があります。
...
繰り返しになりますが、これらは非常に技術的な詳細であり、検索して辿り着いた場合は役立つ可能性があります。
