...
然后，浏览器会向后端发送一个 HTTP `OPTIONS` 请求，如果后端发送适当的 headers 来授权来自不同源（`http://localhost:8080`）的通信，浏览器将允许前端的 JavaScript 向后端发送请求。
...
默认情况下，`CORSMiddleware` 的默认参数实现受限，所以你需要显式地启用特定的源、方法或者 headers，以便浏览器能够在跨域上下文中使用它们。
...
在这种情况下，中间件将拦截传入的请求并进行响应，返回一个使用适当的 CORS headers 的 `200` 或 `400` 响应，出于提供信息的目的。
