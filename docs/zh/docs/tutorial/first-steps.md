...
如果你对原始的 OpenAPI 模式长什么样子感到好奇，FastAPI 会自动生成一份包含了所有 API 描述的 JSON（模式）。
...
在开发 API 时，「路径」是用来分离「关注点」和「资源」的主要手段。
...
!!! tip
    ...
    **FastAPI** 没有强制要求操作有任何特定的含义。
    ...
    比如，当使用 GraphQL 时通常你所有的动作都仅通过 `POST` 操作执行。
