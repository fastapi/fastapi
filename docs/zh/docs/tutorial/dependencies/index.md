...
**"依赖文件注入"** 表示在编程过程中，你的代码（以此处的*路径操作函数*为例）得以声明其运行时所需要的物件以及”依赖文件“的使用。
...
即使您在函数参数中使用`依赖文件`的方法与您使用`躯干`、`请求`的方法一致，`依赖文件`的运行方式略有不同
...
!!! 提示：
    您将会在下一章节看到函数以外的其它文件被用作依赖文件。

每当一个新的请求出现，FastAPI将会处理：
...
这样一来每当您编写一次共享代码**FastAPI**将会负责为您的*路径操作*进行调用操作。
...
依赖文件注入系统可用于制作集成文件与“插件”，但事实上我们*没有必要*去制作“插件”，因为通过使用“依赖项”，我们可以声明无限数量的集成文件和交互文件，它们可用于您的*路径操作功能*

而且依赖项可以非常简单、直观的方式创造出来，您只需要导入需要的Python组件，并用几行代码将它们与您的API函数集成。
