...
如果你正在使用一个第三方库做一些事情（比如：数据库、API、文件系统...），这些操作又不支持使用 `await` （目前大多数数据库都是这样），这种情况你可以像平常那样使用 `def` 声明一个普通的路径操作函数，就像这样：
...
这个执行的时间大多是在等待 <abbr title="Input and Output">I/O</abbr> 操作，因此它们被叫做 "I/O 绑定" 操作。
...
使用 **FastAPI**，您可以利用 Web 开发中常见的并发机制（NodeJS 的主要吸引力）。

并且，您也可以利用并行和多进程（让多个进程并行运行）的优点来处理与机器学习系统中类似 **CPU 绑定** 的工作。
...
所有这些都是 FastAPI（通过 Starlette）的动力，也是它拥有如此令人印象深刻的性能的原因。
...
如果您使用过另一个不以上述方式工作的异步框架，并且您习惯于用普通的 `def` 定义普通的仅计算路径操作函数，以获得微小的性能增益（大约100纳秒），请注意，在 FastAPI 中，效果将完全相反。在这些情况下，最好使用 `async def`，除非路径操作函数内使用执行阻塞的代码。
