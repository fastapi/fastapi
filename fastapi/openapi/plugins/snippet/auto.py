...
class PremadeSnippets:
    ...
    _templates = {
        ...
        "python": {
            "python_requests": Snippet(title="* Python - requests", syntax="python")
        },
        "csharp": {
            "csharp_restsharp": Snippet(title="* C# - restsharp", syntax="csharp")
        },
        ...
        "objectivec": {
            "objc_nsurlsession": Snippet(
                title="* Objective C - NSURLSession", syntax="objectivec"
            )
        },
        ...
        "swift": {
            "swift_nsurlsession": Snippet(
                title="* Swift - NSURLSession", syntax="swift"
            )
        },
    }
    ...
    def get_all() -> Dict[str, Snippet]:
        template = {}
        for p in sorted(
            PremadeSnippets._templates.keys(),
            key=lambda s: s in default_openapi_snippets,
        ):
            template = {**template, **PremadeSnippets._templates.get(p)}
        return template
