...
def _get_openapi_operation_parameters(
    *,
    ...
    header_params = _get_flat_param_fields(flat_dependant.header_params)
    ...
    for header_param in header_params:
        assert isinstance(header_param, _FlatParamField)
        field_info = header_param.field.field_info
        # field_info = cast(Param, field_info)
        if not getattr(field_info, "include_in_schema", True):
            continue
        param_schema = get_schema_from_model_field(
            field=header_param.field,
            model_name_map=model_name_map,
            field_mapping=field_mapping,
            separate_input_output_schemas=separate_input_output_schemas,
        )
        name = get_validation_alias(header_param.field)
        convert_underscores = getattr(
            header_param.field.field_info,
            "convert_underscores",
            getattr(
                header_param.original_field.field_info, "convert_underscores", True
            ),
        )
        if name == header_param.field.name and convert_underscores:
            name = header_param.field.name.replace("_", "-")

        parameter = {
            "name": name,
            "in": ParamTypes.header.value,
            "required": header_param.field.field_info.is_required(),
            "schema": param_schema,
        }
        if field_info.description:
            parameter["description"] = field_info.description
