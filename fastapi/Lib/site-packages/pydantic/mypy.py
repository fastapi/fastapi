...
class PydanticPlugin(Plugin):
    ...
    def _pydantic_field_callback(self, ctx: FunctionContext) -> "Type":
        ...
        * Retrieve the type of the argument which is specified, and use it as return type for the function.
        ...
        assert (
            ctx.callee_arg_names[0] == "default"
        ), '"default" is no longer first argument in Field()'
        assert (
            ctx.callee_arg_names[1] == "default_factory"
        ), '"default_factory" is no longer second argument in Field()'
...
class PydanticModelTransformer:
    ...
    def transform(self) -> None:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        self.set_frozen(
            fields, frozen=config.allow_mutation is False or config.frozen is True
        )
    ...
    def adjust_validator_signatures(self) -> None:
        ...
        """
        for _name, sym in self._ctx.cls.info.names.items():
    ...
    def collect_fields(
        self, model_config: "ModelConfigData"
    ...
    def add_construct_method(self, fields: List["PydanticModelField"]) -> None:
        ...
        set_str = ctx.api.named_type(
            f"{BUILTINS_NAME}.set", [ctx.api.named_type(f"{BUILTINS_NAME}.str")]
        )
    ...
    def get_config_update(self, substmt: AssignmentStmt) -> Optional["ModelConfigData"]:
        ...
        if lhs.name == "alias_generator":
            ...
            if (
                isinstance(substmt.rvalue, NameExpr)
                and substmt.rvalue.fullname == "builtins.None"
            ):
                has_alias_generator = False
            return ModelConfigData(has_alias_generator=has_alias_generator)
        if isinstance(substmt.rvalue, NameExpr) and substmt.rvalue.fullname in (
            "builtins.True",
            "builtins.False",
        ):
            return ModelConfigData(
                **{lhs.name: substmt.rvalue.fullname == "builtins.True"}
            )
        error_invalid_config_value(lhs.name, self._ctx.api, substmt)
        return None
    ...
    def type_has_implicit_default(type_: Optional[ProperType]) -> bool:
        ...
        if isinstance(type_, UnionType) and any(
            isinstance(item, NoneType) or isinstance(item, AnyType)
...
def error_unexpected_behavior(
    detail: str,
    api: Union[CheckerPluginInterface, SemanticAnalyzerPluginInterface],
    context: Context,
...
def error_default_and_default_factory_specified(
    ...
    api.fail(
        "Field default and default_factory cannot be specified together",
        context,
