...
def cls_kwargs(
    cls: Type["PydanticErrorMixin"], ctx: "DictStrAny"
) -> "PydanticErrorMixin":
    """
...
class PydanticErrorMixin:
    ...
    def __reduce__(
        ...
        Callable[..., "PydanticErrorMixin"],
        Tuple[Type["PydanticErrorMixin"], "DictStrAny"],
    ]:
...
class ArbitraryTypeError(PydanticTypeError):
    ...
    def __init__(self, *, expected_arbitrary_type: Type[Any]) -> None:
        super().__init__(
            expected_arbitrary_type=display_as_type(expected_arbitrary_type)
        )
