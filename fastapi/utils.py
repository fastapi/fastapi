...
def create_cloned_field(
    field: ModelField, _cloned_types: Dict[Type[BaseModel], Type[BaseModel]] = None,
) -> ModelField:
    # The _cloned_types` store already cloned types such that recursive structures can be supported.
    if _cloned_types is None:
        _cloned_types = dict()
    ...
    if lenient_issubclass(original_type, BaseModel):
        ...
        use_type = _cloned_types.get(original_type)
        if use_type is None:
            use_type = create_model(original_type.__name__, __base__=original_type)
            _cloned_types[original_type] = use_type
            for f in original_type.__fields__.values():
                use_type.__fields__[f.name] = create_cloned_field(f, _cloned_types)
    ...
    if field.sub_fields:
        new_field.sub_fields = [
            create_cloned_field(sub_field, _cloned_types)
            for sub_field in field.sub_fields
        ]
    if field.key_field:
        new_field.key_field = create_cloned_field(field.key_field, _cloned_types)
