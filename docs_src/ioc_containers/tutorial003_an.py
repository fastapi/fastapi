...
@lru_cache
def get_client_settings(user: UserDep) -> ClientSettings:
...
async def get_city_client(settings: ClientSettingsDep) -> Client:
    return CityClient(settings=settings)


async def get_village_client(settings: ClientSettingsDep) -> Client:
    return VillageClient(settings=settings)


CityClientDep = Annotated[Client, Depends(get_city_client)]

VillageClientDep = Annotated[Client, Depends(get_village_client)]
