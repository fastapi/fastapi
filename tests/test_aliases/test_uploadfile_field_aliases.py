...
async def required_field_alias(file: UploadFile = File(alias="file_alias")):
...
async def list_field_alias(files: List[UploadFile] = File(alias="files_alias")):
...
async def workaround(
    file: UploadFile = File(alias="file_alias", validation_alias="file_alias"),
):
    return {"file_size": file.size}
