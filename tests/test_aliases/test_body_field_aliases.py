...
async def required_field_alias(
    param: str = Body(alias="param_alias", embed=True),
):
    return {"param": param}


def test_required_field_alias_():
...
async def optional_field_alias(
    param: Optional[str] = Body(None, alias="param_alias", embed=True),
):
    return {"param": param}
...
async def list_field_alias(param: List[str] = Body(alias="param_alias", embed=True)):
    return {"param": param}
...
async def optional_list_field_alias(
    param: Optional[List[str]] = Body(None, alias="param_alias", embed=True),
):
    return {"param": param}
...
async def required_field_validation_alias(
    param: str = Body(validation_alias="param_val_alias", embed=True),
):
    return {"param": param}
...
async def optional_field_validation_alias(
    param: Optional[str] = Body(None, validation_alias="param_val_alias", embed=True),
):
    return {"param": param}
...
async def list_field_validation_alias(
    param: List[str] = Body(validation_alias="param_val_alias", embed=True),
):
    return {"param": param}
...
async def optional_list_field_validation_alias(
    param: Optional[List[str]] = Body(
        None, validation_alias="param_val_alias", embed=True
    ),
):
    return {"param": param}
...
async def required_field_alias_and_validation_alias(
    param: str = Body(
        alias="param_alias", validation_alias="param_val_alias", embed=True
    ),
):
    return {"param": param}
...
async def optional_field_alias_and_validation_alias(
    param: Optional[str] = Body(
        None, alias="param_alias", validation_alias="param_val_alias", embed=True
    ),
):
    return {"param": param}
...
# =====================================================================================
# Body(alias=..., validation_alias=...)  # alias == validation_alias
# The only working solution (current workaround)
# TODO: remove when issue with Body field aliases is fixed


@app.post("/workaround", operation_id="workaround")
async def workaround(
    param: str = Body(alias="param_alias", validation_alias="param_alias", embed=True),
):
    return {"param": param}
...
def test_workaround_by_name():
    client = TestClient(app)
    resp = client.post("/workaround", json={"param": "123"})
    assert resp.status_code == 422
    detail = resp.json()["detail"]
    assert detail[0]["msg"] == "Field required"
    assert "param_alias" in detail[0]["loc"]
...
def test_workaround_by_alias():
    client = TestClient(app)
    resp = client.post("/workaround", json={"param_alias": "123"})
    assert resp.status_code == 200
    assert resp.json() == {"param": "123"}
