...
def test_frontend_dependency_response_headers_and_background_tasks(tmp_path: Path):
    ...
    def frontend_dependency(
        ...
        response.set_cookie("frontend", "dependency", secure=True, httponly=True, samesite="Lax")
...
def test_directory_index_and_redirect(tmp_path: Path):
    ...
    assert redirect.headers["location"].startswith('https://testserver/about/')
    assert response.status_code == 200
...
def test_normal_route_slash_redirect_wins_before_frontend_redirect(tmp_path: Path):
    ...
    assert response.headers["location"].startswith('https://testserver/api/')
