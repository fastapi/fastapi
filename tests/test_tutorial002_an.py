...
import builtins
import asyncio
import docs_src.background_tasks.tutorial002_an as tutorial002_an
...
def test_background_tasks_execution_order_manual(monkeypatch):
    ...
    response = asyncio.run(tutorial002_an.send_notification("order@example.com", fake_bg, result_query))
    assert response == {"message": "Message sent"}
    
    assert len(fake_bg.tasks) == 2
    expected_task_args = [("found query: orderTest\n",), ("message to order@example.com\n",)]
...
def test_openapi_schema_contains_query_parameter():
    ...
    q_parameters = [param for param in parameters if param.get("name") == "q" and param.get("in") == "query"]
    assert len(q_parameters) == 1, "Expected one query parameter named 'q' in the OpenAPI schema"
def test_dependency_exception(monkeypatch):
    ...
    def failing_get_query(background_tasks: tutorial002_an.BackgroundTasks, q: str = None):
        raise Exception("Simulated dependency failure")
    tutorial002_an.app.dependency_overrides[tutorial002_an.get_query] = failing_get_query
    client = TestClient(tutorial002_an.app, raise_server_exceptions=False)
...
def test_openapi_schema_contains_email_parameter():
    ...
    post_schema = openapi_schema["paths"]["/send-notification/{email}"]["post"]
    parameters = post_schema.get("parameters", [])
    email_parameters = [param for param in parameters if param.get("name") == "email" and param.get("in") == "path"]
    assert len(email_parameters) == 1, "Expected one path parameter named 'email' in the OpenAPI schema"
    email_param = email_parameters[0]
    assert email_param.get("required") is True, "Path parameter 'email' should be required"
    schema = email_param.get("schema", {})
    assert schema.get("type") == "string", "Path parameter 'email' should be of type string"
...
def test_send_notification_with_special_whitespace(monkeypatch):
    ...
    response = client.post("/send-notification/special@example.com", params={"q": special_query})
    assert response.status_code == 200
    assert response.json() == {"message": "Message sent"}
