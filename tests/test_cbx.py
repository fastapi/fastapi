...
class UserCBV:
    ...
    async def put(self, user_id: int, name: str):
        if not self.db.update_user(user_id, name):
            raise HTTPException(
                status_code=404, detail="User not found, update failed"
            )  # pragma: no cover

    def delete(self, user_id: int):
        if not self.db.delete_user(user_id):
            raise HTTPException(
                status_code=404, detail="User not found, delete failed"
            )  # pragma: no cover
